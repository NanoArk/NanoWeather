...
public class Tesseract implements ITesseract {
    ...
    */
   @Override
   public String doOCR(int xsize, int ysize, ByteBuffer buf, Rectangle rect, int bpp)
      throws TesseractException {
    ...
    */
   @Override
   public String doOCR(int xsize, int ysize, ByteBuffer buf, String filename, Rectangle rect,
   ...
   protected String getOCRText(String filename, int pageNum) {
      ...
      Pointer utf8Text = renderedFormat == RenderedFormat.HOCR
         ? api.TessBaseAPIGetHOCRText(handle, pageNum - 1) : api.TessBaseAPIGetUTF8Text(handle);
         ...
         case PDF:
            ...
            if(renderer == null) {
               ...
               api.TessResultRendererInsert(renderer,
                  api.TessPDFRendererCreate(outputbase, dataPath));
            }
            break;
         case BOX:
            if(renderer == null) {
    ...
    */
   @Override
   public List<Rectangle> getSegmentedRegions(BufferedImage bi, int pageIteratorLevel)
      throws TesseractException {
      init();
      setTessVariables();
      try {
         ...
         setImage(bi, null);
         Boxa boxes = api.TessBaseAPIGetComponentImages(handle, pageIteratorLevel, TRUE, null,
            null);
